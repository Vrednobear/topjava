...
public class MealsUtil {
    ...
    public static List<MealTo> getFilteredTosForUser(Collection<Meal> meals, int caloriesPerDay, int userId,
    ...
    public static MealTo getTo(Collection<Meal> meals, Meal initialMeal, int caloriesPerDay, int userId) {
        ...
        if(caloriesSumByDate.isEmpty()) return null;

        return meals.stream()
                .filter(meal -> meal.getId().equals(initialMeal.getId()))
                .map(meal -> createTo(meal, caloriesSumByDate.get(meal.getDate()) > caloriesPerDay))
